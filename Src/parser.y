	...
	| undefsym D_SET reg
		{
			...
			sprintf($1.sym->literal, "R%ld", $3.val.v);
			$1.sym->type = SET;
			$1.sym->value = 0;
			$$.value = 0;
		}
	| undefsym D_SET areg
		{
			...
			sprintf($1.sym->literal, "AR%ld", $3.val.v);
			$1.sym->type = SET;
			$1.sym->value = 0;
			$$.value = 0;
		}
...
expr:
	...
	| SYMBOL
		{
			...
			if( $1.sym->type != UNDEF )
			{
				if($1.sym->segment!=NULL) // Does this properly takes care of $1.sym->segment==NULL?  See below.
				{
					...
					{
						sprintf(tmpexp,"0x%04lX;",$1.sym->value);
	...
	| VALUE
		{
			sprintf(tmpexp,"0x%04lX;",$1.val.v);
			strcat(symexp,tmpexp);
			$$.val.k = 0;
			$$.val.s = 0;
	...
	| data8 BITPOS ',' C
		{
			if( pass2 )
			{
				if($1.val.s==1)
				{
					sprintf($$.mode.str1, "bit(%s%04lX;.;)", $1.val.rel, $2.value);
					$$.mode.str2[0]=' ';
					$$.mode.str2[1]=0;	
				}
				else
	...
	expr BITPOS
		{
			...
			if( pass2 )
			{
				if($1.val.s==1)
				{
					sprintf($$.val.rel, "bit(%s%04lX;.;)", symexp, $2.value);
					$$.val.s=1;
				}
				else
				{
	...
	expr
		{
			...
			else
			{
				...
				set_b1($$.mode, $1.val.v / 0x100 ); // Doesn't work on macOS / Linux
				set_b2($$.mode, ($1.val.v & 0x00ff ) ); // Works
				$$.val.v=$1.val.v;
